/* BaiLongma — Hotspot Earth (3D 地球热点) _bl 适配版
   Three.js 程序化地球 + 云层 + 大气辉光 + 城市标记 + 跑马灯
   基于 BaiLongma 设计思想，纯前端，零后端API依赖
   ──────────────────────────────────────────────────── */

const BaiLongmaHotspot_bl = (() => {
  'use strict';
  ...
  // ── 经纬度 → 3D 坐标 ──
  function latLngToVector3_bl(lat, lng, radius) {
  ...
  // ── 程序化地球纹理 ──
  function createEarthTexture_bl() {
  ...
  // ── 云层纹理 ──
  function createCloudTexture_bl() {
    const canvas = document.createElement('canvas');
    canvas.width = 512;
    canvas.height = 256;
    const ctx = canvas.getContext('2d');
  ...
  function init_bl(containerEl) {
    ...
    for (const city of CITIES) {
      const pos = latLngToVector3_bl(city.lat, city.lng, 1.55);
      const marker = createCityMarker_bl(pos, city.heat);
  ...
  // ── 城市标记 (光柱+光环) ──
  function createCityMarker_bl(position, heat) {
  ...
  function loadTickerData_bl() {
    ...
    // 更新跑马灯
    updateTicker_bl(tickerEl);
  }

  function updateTicker_bl(el) {
    if (!el) return;
  ...
  // ── 渲染循环 ──
  function animate_bl() {
    requestAnimationFrame(animate_bl);
