/* BaiLongma — Voice Panel (声波点云球) _bl 适配版
...
const BaiLongmaVoice_bl = (() => {
  ...
  // ── 初始化 ──
  function init_bl(containerId) {
    panel = document.getElementById(containerId);
    if (!panel) {
      console.warn('[BaiLongmaVoice_bl] container not found');
      return;
    }
    ...
    resize_bl();
    generateFibPoints_bl(CONFIG.count);
    bindEvents_bl();
    startAnim_bl();
  }
  ...
  // ── Fibonacci 球面采样 ──
  function generateFibPoints_bl(n) {
    points = [];
  ...
  // ── 3D → 2D 投影 ──
  function project_bl(p, angleX, angleY) {
  ...
  // ── 获取当前颜色 ──
  function getColor_bl(mode, amplitude) {
  ...
  function draw_bl() {
    ...
    const color = getColor_bl(state.mode, state.amplitude);
    const projected = points.map(p => ({
      ...project_bl(p, state.angleX, state.angleY),
    }));
    // 按 z 排序
    ...
    state.animId = requestAnimationFrame(draw_bl);
  }

  function startAnim_bl() {
    if (state.animId) cancelAnimationFrame(state.animId);
    draw_bl();
  }

  // ── 状态切换 ──
  function setMode_bl(mode) {
    state.mode = mode;
  ...
  function updateAmplitude_bl(amp) {
    ...
    if (state.mode === 'speaking' || state.mode === 'thinking') {
      if (amp > CONFIG.duckThreshold) {
        ...
        if (state.ducking) {
          ...
          // 自动恢复
          setTimeout(() => {
            if (!state.ducking) restoreVolume_bl();
          }, CONFIG.recoveryMs);
        }
      }
    }
  ...
  function duckVolume_bl() {
    setMode_bl('interrupted');
    // 降低浏览器 TTS 音量
    if (window._blUtterance) {
      window._blUtterance.volume = Math.max(0.1, window._blUtterance.volume - 0.3);
    }
  }

  function restoreVolume_bl() {
    if (window._blUtterance) {
      window._blUtterance.volume = 1.0;
    }
  ...
  function interrupt_bl() {
    ...
    state.ducking = false;
    // 标记中断
    if (typeof AtlasStream !== 'undefined') {
  ...
  function speak_bl(text, lang) {
    if (!window.speechSynthesis) {
      console.warn('[BaiLongmaVoice_bl] speechSynthesis not available');
      return;
    }
    ...
    window._blUtterance = utterance;

    utterance.onstart = () => { setMode_bl('speaking'); };
    utterance.onend = () => {
      window._blUtterance = null;
      if (!state.interrupted) setMode_bl('idle');
    };
    utterance.onerror = () => {
      window._blUtterance = null;
      setMode_bl('error');
      setTimeout(() => setMode_bl('idle'), 1500);
    };
  ...
  // ── 事件 ──
  function bindEvents_bl() {
    window.addEventListener('resize', resize_bl);
  }
  ...
  return {
    init: init_bl,
    setMode: setMode_bl,
    updateAmplitude: updateAmplitude_bl,
    speak: speak_bl,
    stop: stop_bl,
    destroy: destroy_bl,
  };
